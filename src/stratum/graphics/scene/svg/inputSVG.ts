...
export class InputSVG extends InputElement2D {
    ...
    render(): void {
        ...
        if (this._prevFontVer !== this.font._ver) {
            this._prevFontVer = this.font._ver;
            const f = this.font._tool as FontSVG | null;
            if (f) {
                this.st.setProperty("font", f.toCSSString());
            } else {
                this.st.removeProperty("font");
            }
        }
