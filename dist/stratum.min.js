...
`;function Ds(e,t,s){let r=e.length,i=`${de}.${gs}();`;e.push(`${de}.${bs}(${t});`,`return -${r};`,`case -${r}:`,s?`var ${t} = ${i}`:i)}var As=class{get(t){return this.v.get(t.toUpperCase())}constructor(t){this.v=new Map(t==null?void 0:t.map((s,r)=>[s.name.toUpperCase(),[s,r]]))}result(){return[...this.v.values()].map(t=>t[0])}},Rs=class{constructor(t,s,r,i,n){this.res=t;this.vars=s;this.lib=r;this.missingFuncs=i;this.innerFuncs=n;this.varCount=0;this._hasBug=!1}reset(){return this.varCount=0,this}hasBug(){return this._hasBug}arr(t,s){let r=this.vars.get(s);if(!r)throw Error(`\u041D\u0435\u0438\u0437\u0432\u0435\u0441\u0442\u043D\u0430\u044F \u043F\u0435\u0440\u0435\u043C\u0435\u043D\u043D\u0430\u044F: ${s}`);let i=H[r[0].type];return`${t?_e:Te}${i}[${Ze}[${r[1]}]]`}derefer(t){if(t.first.type!=="var"||t.rest.length>0)throw Error("\u0414\u043E\u043B\u0436\u043D\u043E \u0431\u044B\u0442\u044C \u0438\u043C\u044F \u043F\u0435\u0440\u0435\u043C\u0435\u043D\u043D\u043E\u0439");let s=t.first.name,r=this.vars.get(s);if(!r)throw Error(`\u041D\u0435\u0438\u0437\u0432\u0435\u0441\u0442\u043D\u0430\u044F \u043F\u0435\u0440\u0435\u043C\u0435\u043D\u043D\u0430\u044F: ${s}`);return[`${t.first.isNew?_e:Te}${H[r[0].type]}`,`${Ze}[${r[1]}]`]}parseOperand(t){var s,r;switch(t.isNew&&t.type!=="var"&&(this._hasBug=!0),t.type){case"float":case"handle":return t.value.toString();case"string":return"`"+t.value.split("\\").join("\\\\").split("`").join("\\`")+"`";case"var":{let i=t.name.toUpperCase();return(r=(s=x[i])==null?void 0:s.toString())!=null?r:this.arr(t.isNew,i)}case"call":{let i=this.parseCall(t),n=this.put(i.f);if(i.blocking){let o=Array.from({length:this.varCount-1},(a,u)=>`t${u}`);if(o.length>0&&this.res.push(`${de}.${ds}([${o.join(",")}]);`),Ds(this.res,n,!0),o.length>0){let a=`var ctx = ${de}.${_s}();`,u=o.map((h,p)=>`var ${h}=ctx[${p}];`).join("");this.res.push(a,u)}}return n}case"-":return`(-${this.parseOperand(t.op)})`;case"!":return`(!${this.parseOperand(t.op)})`;case"subexpr":return`(${this.parseExpr(t.expr)})`}}put(t){let s=`t${this.varCount++}`;return this.res.push(`var ${s}=${t};`),s}parseCall(t){let s=t.name.toUpperCase(),r=t.args.map(o=>this.parseExpr(o)),i=qe.get(s);if(i){if(i.inline!=="template")return i.inline==="inner"&&this.innerFuncs.add(i.f),L(L({},i),{f:`${i.f}(${r})`});let o=i.f,a=0;for(;a<r.length;){let h=o.replace(`#${a}`,r[a]);if(o===h)throw Error(`\u0421\u043B\u0438\u0448\u043A\u043E\u043C \u043C\u043D\u043E\u0433\u043E \u0430\u0440\u0433\u0443\u043C\u0435\u043D\u0442\u043E\u0432 \u0432 \u0444\u0443\u043D\u043A\u0446\u0438\u0438 ${t.name}: ${r.length}, \u043C\u0430\u043A\u0441\u0438\u043C\u0443\u043C - ${a})`);o=h,++a}let u=o;for(;(u=o.replace(`#${a}`,"undefined"))!==o;)o=u,++a;return L(L({},i),{f:o})}if(s==="INC"||s==="DEC"){let o=this.derefer(t.args[0]),a=t.args.length<2?1:this.parseExpr(t.args[1]);return{f:`${o[0]}[${o[1]}]${s==="INC"?"+":"-"}=${a}`}}if(s==="GETTIME"){let o=t.args.map(a=>this.derefer(a));return{f:`${se}.${Ts}(${o})`}}if(s==="GETDATE"){let o=t.args.map(a=>this.derefer(a));return{f:`${se}.${Ss}(${o})`}}if(s==="GETACTUALSIZE2D"){let o=t.args.slice(0,2).map(u=>this.parseExpr(u)),a=t.args.slice(2).map(u=>this.derefer(u));return{f:`${se}.${Es}(${o},${a})`}}if(s==="GETVARINFO"){let o=t.args.slice(0,2).map(u=>this.parseExpr(u)),a=t.args.slice(2).map(u=>this.derefer(u));return{f:`${se}.${ys}(${o},${a})`}}if(s==="GETMOUSEPOS"){let o=this.parseExpr(t.args[0]),a=this.derefer(t.args[1]),u=this.derefer(t.args[2]);return{f:`${se}.${vs}(${o},${a},${u})`}}if(s==="GETBITMAPSRCRECT2D"){let o=t.args.slice(0,2).map(u=>this.parseExpr(u)),a=t.args.slice(2).map(u=>this.derefer(u));return{f:`${se}.${ws}(${o},${a})`}}return s==="RANDOMIZE"?{f:""}:(this.lib.hasModel(t.name)||this.missingFuncs.set(s,t.name),{f:`${de}.${fs}(${Ve}, "${t.name}"${r.length>0?","+r:""})`,blocking:!0})}parseExpr(t){let s=this.parseOperand(t.first),r=t.rest.map(i=>i.action==="/"?`/(${this.parseOperand(i.operand)} || Infinity)`:i.action+this.parseOperand(i.operand));return s+r.join("")}},Ps=class{constructor(){this.bblocks=[]}pushBlock(t){return this.bblocks.push(t),t}popBlock(t){let s=this.bblocks.pop();if(!s||s.expect!==t)throw Error(`\u041E\u0436\u0438\u0434\u0430\u043B\u043E\u0441\u044C ${t}`);return s}lastBlock(){return this.bblocks.length>0?this.bblocks[this.bblocks.length-1]:null}getBreakID(){for(let t=this.bblocks.length-1;t>=0;--t){let s=this.bblocks[t];if(s.canBreak)return s.end}return Ae}},Ut=class{constructor(t,s,r){this.lib=s;this._hasEquals=!1;this._hasBug=!1;this._hasFuncDecl=!1;this.labelCnt=0;this.missingFuncs=new Map;this.res=[];this.b=new Ps;this.innerFuncs=new Set;this._vars=new As(r),this.res=[`var code = ${pt};`,"while(true) {","switch(code) {","case 0:"];let i=new Rs(this.res,this._vars,this.lib,this.missingFuncs,this.innerFuncs);for(let o=0;o<t.length;++o)try{let a=t[o];this.parseCodeline(a,i.reset());let u=a.then;for(;u;)this.parseCodeline(u,i.reset()),u=u.then}catch(a){throw Error(a.message+" \u0432 \u0441\u0442\u0440\u043E\u043A\u0435 "+(o+1))}this._hasBug=i.hasBug();let n=this.b.lastBlock();if(n)throw Error(`\u041E\u0436\u0438\u0434\u0430\u043B\u043E\u0441\u044C ${n.expect}`);this.res.push("}",`return ${Ae};`,"}")}comment(t){this.res.push("//"+t)}makeJump(t){return`{ code = ${t}; continue; }`}parseCodeline(t,s){switch(t.type){case":=":{this.res.push(`${s.arr(!0,t.to)} = ${s.parseExpr(t.expr)};`);break}case"callChain":{for(let i=0;i<t.functions.length;++i){let n=t.functions[i],o=s.reset().parseCall(n);o.blocking?Ds(this.res,o.f,!1):this.res.push(`${o.f};`)}break}case"if":{this.comment("if");let i=this.b.pushBlock({canBreak:!1,end:++this.labelCnt,expect:"endif",haveElse:!1});this.res.push(`if(!(${s.parseExpr(t.expr)})) ${this.makeJump(i.end)}`);break}case"else":{let i=this.b.lastBlock();if(!i||i.expect!=="endif"||i.haveElse)throw Error("else \u0434\u043E\u043B\u0436\u0435\u043D \u043D\u0430\u0445\u043E\u0434\u0438\u0442\u044C\u0441\u044F \u043F\u043E\u0441\u043B\u0435 if");i.haveElse=!0;let n=i.end;i.end=++this.labelCnt,this.res.push(this.makeJump(i.end)),this.comment("else"),this.res.push(`case ${n}:`);break}case"endif":{this.comment("endif"),this.res.push(`case ${this.b.popBlock("endif").end}:`);break}case"while":{this.comment("while");let i=this.b.pushBlock({expect:"endwhile",start:++this.labelCnt,end:++this.labelCnt,canBreak:!0});this.res.push(`case ${i.start}:`),this.res.push(`if(!(${s.parseExpr(t.expr)})) ${this.makeJump(i.end)}`);break}case"endwhile":{let i=this.b.popBlock("endwhile");this.res.push(this.makeJump(i.start)),this.comment("endwhile"),this.res.push(`case ${i.end}:`);break}case"do":{this.comment("do");let i=this.b.pushBlock({expect:"until",start:++this.labelCnt,end:++this.labelCnt,canBreak:!0});this.res.push(`case ${i.start}:`);break}case"until":{let i=this.b.popBlock("until");this.res.push(`if(${s.parseExpr(t.expr)}) ${this.makeJump(i.start)}`),this.comment("until"),this.res.push(`case ${i.end}:`);break}case"switch":{this.comment("switch"),this.b.pushBlock({expect:"endswitch",end:++this.labelCnt,next:null,canBreak:!0,hasCase:!1});break}case"endswitch":{this.comment("endswitch");let i=this.b.popBlock("endswitch");i.next!==null&&this.res.push(`case ${i.next}:`),this.res.push(`case ${i.end}:`);break}case"case":{let i=this.b.lastBlock();if(!i||i.expect!=="endswitch")throw Error("case \u0434\u043E\u043B\u0436\u0435\u043D \u043D\u0430\u0445\u043E\u0434\u0438\u0442\u044C\u0441\u044F \u0432\u043D\u0443\u0442\u0440\u0438 \u0431\u043B\u043E\u043A\u0430 switch");i.hasCase=!0,i.next!==null?(this.res.push(this.makeJump(i.end)),this.comment("case"),this.res.push(`case ${i.next}:`)):this.comment("case"),i.next=++this.labelCnt,this.res.push(`if(!(${s.parseExpr(t.expr)})) ${this.makeJump(i.next)}`);break}case"default":{let i=this.b.lastBlock();if(!i||i.expect!=="endswitch")throw Error("default \u0434\u043E\u043B\u0436\u0435\u043D \u043D\u0430\u0445\u043E\u0434\u0438\u0442\u044C\u0441\u044F \u0432\u043D\u0443\u0442\u0440\u0438 \u0431\u043B\u043E\u043A\u0430 switch");if(!i.hasCase)throw Error("\u041E\u0436\u0438\u0434\u0430\u043B\u043E\u0441\u044C case");i.next!==null?(this.res.push(this.makeJump(i.end)),this.comment("default"),this.res.push(`case ${i.next}:`)):this.comment("default"),i.next=null;break}case"break":this.comment("break");let r=this.b.getBreakID();this.res.push(r===Ae?`return ${Ae};`:this.makeJump(r));break;case"varsDec":break;case"function":this._hasFuncDecl=!0;break;case"return":break;case"=":this._hasEquals=!0;break;default:throw Error(`\u041D\u0435\u0438\u0437\u0432\u0435\u0441\u0442\u043D\u044B\u0439 \u043E\u043F\u0435\u0440\u0430\u0442\u043E\u0440: ${t.type}`)}}result(){return this.res}vars(){return this._vars.result()}innerFunctions(){return[...this.innerFuncs]}hasEquations(){return this._hasEquals}hasFunctionDecl(){return this._hasFuncDecl}hasBug(){return this._hasBug}};var Wa=`function arccos(a) {
    ...
    return res;
}`,ja=`function compareI(s1, s2, n) {
    return s1.substring(0, n) === s2.substring(0, n) ? 1 : 0;
}`,Xa=`function getAngleByXY(x, y) {
    ...
    return idx;
}`,qa=`function right(a, n) {
    return a.substring(a.length - n);
}`,Qa=`function round(a, b) {
    var pw = Math.ceil(10 ** b);
    return Math.round(a * pw - Number.EPSILON) / pw || 0;
}`,Os=new Map([["arccos",Wa],["arcsin",Ha],["change",Ga],["compareI",ja],["getAngleByXY",Xa],["lg",Ya],["ln",Ka],["log",za],["pos",Za],["right",qa],["round",Qa]]);var Wt="'",Ht='"';function Bs(e){let t="",s=null;for(let r=0;r<e.length;++r){let i=e[r];if(i===Wt&&s!==Ht?s=s?null:Wt:i===Ht&&s!==Wt&&(s=s?null:Ht),s){t+=i;continue}if(i==="/"){let n=e[r+1];if(n==="*"){for(r+=2;r<e.length&&(e[r]!=="*"||e[r+1]!=="/");)++r;++r;continue}if(n==="/"){for(++r;r<e.length&&e[++r]!==`
`;);t[t.length-1]!==`
`&&(t+=`
`);continue}}if(i===";"||i===`
`){t[t.length-1]!==`
